...
vault kv put -mount=security-codedesignplus-keyvalue ms-filestorage \
...
vault kv get -mount=security-codedesignplus-keyvalue ms-filestorage

# Write database configuration
echo "$newlines"
echo "${BLUE}7. Writing database configuration...${NC}"
vault write security-codedesignplus-database/config/db-ms-filestorage \
    plugin_name=mongodb-database-plugin \
    allowed_roles="ms-filestorage-mongo-role" \
    connection_url="mongodb://{{username}}:{{password}}@mongo:27017/admin?ssl=false" \
    username="admin" \
    password="password"

vault write security-codedesignplus-database/roles/ms-filestorage-mongo-role \
    db_name=db-ms-filestorage \
    creation_statements='{ "db": "admin", "roles": [{ "role": "readWrite", "db": "db-ms-filestorage" }] }' \
    default_ttl="1h" \
    max_ttl="24h"

vault read security-codedesignplus-database/creds/ms-filestorage-mongo-role
...
vault write security-codedesignplus-rabbitmq/roles/ms-filestorage-rabbitmq-role \
    vhosts='{"/":{"write": ".*", "read": ".*", "configure": ".*"}}'

vault read security-codedesignplus-rabbitmq/creds/ms-filestorage-rabbitmq-role