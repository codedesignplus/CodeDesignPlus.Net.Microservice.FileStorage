...
public class FileStorageControllerTest : ServerBase<Program>, IClassFixture<Server<Program>>
{
    ...
    public async Task UploadFile_ReturnOk()
    {
        ...
        Assert.True(Path.Exists(Path.Combine(path, tenant.ToString(), "test", "fake.txt")));
    }
    ...
    public async Task Download_ContentDisposition_Inline_Attachment(string contentDisposition)
    {
        ...
        var responseUpload = await this.RequestAsync("http://localhost/api/FileStorage/upload", content, HttpMethod.Post, tenant);

        Assert.NotNull(responseUpload);
        Assert.Equal(HttpStatusCode.OK, responseUpload.StatusCode);
